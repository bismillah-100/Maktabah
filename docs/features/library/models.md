...
* **String Interning (`StringInterner.shared.intern`)**: Menghemat jejak memori pada puluhan ribu kitab yang memiliki string pengulangan serupa.
...
* **Tipe Khusus (`pdfCs`)**:
    * `pdfCs == 3`: Menandakan kitab *Multi-Language*.
...
### B. `CategoryData` (`CategoryData.swift`)
Representasi hierarki kategori pohon (*nested category tree*):
...
* **Pengumpulan Rekursif**: Menyediakan `allBooks` dan `allSubcategories` yang menjelajahi seluruh turunan hierarki secara rekursif.

---

### C. `Muallif` (`Muallif.swift`)
...
* **`BookMetadata`**: Kapsulasi skema internal kitab untuk kebutuhan verifikasi sebelum peleburan ke arsip.

### B. Status Pembaruan (`UpdateStatus`)
...
### A. `LibraryFilterMode` (`LibraryFilterMode.swift`)
...
### B. `LibraryUpdate` (`LibraryUpdate.swift`)
