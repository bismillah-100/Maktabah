...
`AnnotationTreeBuilder` adalah komponen pengelola hierarki data pohon (*tree data structure*) yang menjembatani data mentah dari `AnnotationStore` ke komponen UI sidebar (`NSOutlineView` di macOS dan tampilan list bertingkat di iOS).

Komponen ini bertanggung jawab untuk:

1. Mengelompokkan (*grouping*) ribuan catatan anotasi ke dalam hierarki pohon logis sesuai preferensi pengguna.
2. Memproses mutasi data secara asinkron di antrean terisolasi (`treeQueue`).
3. Menghitung perubahan struktural (*diffing*) secara presisi agar UI dapat melakukan animasi baris secara efisien tanpa *full reload*.
4. Mengelola penyortiran dan penyaringan kitab yang belum terunduh.
...
Menyusun anotasi berdasarkan induk kitabnya:
```text
Root
...
Ketika terjadi penambahan, pengubahan, atau penghapusan anotasi, `AnnotationTreeBuilder` tidak membangun ulang seluruh pohon dari nol. Sebaliknya, ia menghitung delta perubahan dan memancarkannya melalui Combine Publisher:
...
## 4. Manipulasi Pohon & Filter Kitab Hilang

`AnnotationTreeBuilder` juga menangani kondisi khusus ketersediaan kitab lokal:

* **Penyortiran Dinamis (`AnnotationSortOption`)**: Mengurutkan anak-anak node berdasarkan tanggal pembuatan (`createdAt`), posisi kitab (`page`/`part`), atau judul secara *ascending* maupun *descending*.
* **Observasi Kitab Hilang (`hideMissingBookAnnotations`)**: Memantau `UserDefaults`. Jika opsi ini aktif, anotasi yang kitabnya belum diunduh di perangkat pengguna akan otomatis disaring keluar dari pohon tampilan secara reaktif.
