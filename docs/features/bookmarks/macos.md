...
Dokumentasi ini menguraikan arsitektur antarmuka native macOS untuk modul markah, pengelolaan komponen `NSOutlineView`, orkestrasi pembaruan diferensial (*batch diffing*), interaksi seret-dan-lepas (*drag-and-drop*), serta optimasi rendering berbasis AppKit pada direktori `Source/Features/Bookmarks/macOS/`.
...
*   `bookmarkChild`: Sel `ResultCellView` yang menampilkan ikon kaca pembesar / mode pencarian, judul kueri, dan jumlah item halaman yang cocok.

---

## 3. Mekanisme Batch Diffing (`applyTreeChange`)

Untuk menghindarkan pengguna dari kedipan visual (*flickering*) dan kehilangan status baris yang sedang dipilih atau diekspansi saat data berubah, sistem tidak menggunakan `outlineView.reloadData()` untuk mutasi normal, melainkan menerapkan pembaruan diferensial presisi tinggi:
...
### Animasi Pembaruan Simpul Hasil (`applyResultTreeChange`)

Karena simpul hasil (`ResultNode`) dan subfolder (`FolderNode`) berada di bawah wadah `children` yang sama di mata `NSOutlineView`, indeks hasil pencarian wajib digeser sebesar jumlah subfolder yang ada:
...
Sistem mendukung pemindahan folder dan hasil kueri antar-direktori menggunakan protokol seret-dan-lepas AppKit.

### Validasi Pencegahan Siklus (Cycle Detection)

Saat folder diseret, sistem memvalidasi calon folder tujuan untuk mencegah pengguna memasukkan folder ke dalam dirinya sendiri atau ke dalam rantai keturunannya:
...
!!! note "Pembersihan Status Pencarian"
    Saat kolom pencarian dikosongkan, `isSearching` disetel kembali ke `false`, dan `NSOutlineView` memuat ulang pohon hierarki utuh dengan status ekspansi folder sebelumnya tetap terjaga berkat properti `autosaveExpandedItems = true`.
