...
## 3. Alur Pemuatan Halaman (*Page Loading Pipeline*): Dari Aksi Navigasi ke TextView
...
### B. IbarotTextView (Class) - *Subclass* NSTextView
...
### E. Navigasi Terisolasi & Perekaman Histori Bersyarat (IbarotTextVC)

`IbarotTextVC` menyediakan API navigasi asinkron tingkat tinggi yang aman dari *race condition* untuk membuka anotasi dan riwayat:

* **`openAnnotation(_ annotation: Annotation) async throws`**: Membatalkan `pendingRestoreTask`, memastikan kitab terkait terbuka, berpindah ke `contentId`, dan menginstruksikan perender teks untuk menggulir serta menyorot anotasi.
* **`openHistory(book: BooksData, contentId: Int?) async throws`**: Membuka buku dan melompat ke posisi bacaan terakhir.
* **Perekaman Histori Bersyarat**: Pembaruan seleksi buku ke `LibraryViewModel` membedakan antara aksi navigasi aktif dan pemuatan pratinjau:
  ```swift
  ...
  ```
  `handleBookSelection` (yang mengunggah riwayat ke CloudKit) hanya dipanggil jika `recordHistory == true`, mencegah pencatatan histori palsu saat sekadar memulihkan state jendela.

### F. Pencegahan Retain Cycle & Siklus Hidup Observer

Untuk mencegah kebocoran memori (*memory leaks*) pada controller tampilan:

* **Tangkapan `[weak self]`**: Seluruh *closure* `NotificationCenter` dan KVO pada `ViewerSplitVC` (pengamat tinggi baris, penampilan, warna latar, harakat, dan font), `SidebarVC` (pengamat tab bar dan jendela), serta `IbarotTextVC` wajib menggunakan `[weak self]`.
* **Pembersihan Independen di `deinit`**: Pada `ViewerSplitVC`, observer dihapus secara individual tanpa bergantung pada keberadaan seluruh token sekaligus:
  ```swift
  deinit {
      ...
      if let tasykilObserver { NotificationCenter.default.removeObserver(tasykilObserver) }
      if let lineHeightObservation { NotificationCenter.default.removeObserver(lineHeightObservation) }
      // ...
  }
  ```
* **Otomasi `NotificationToken`**: `IbarotTextVC` memanfaatkan koleksi `observerTokens: [NotificationToken]` yang secara otomatis mencabut registrasi observer dari `NotificationCenter` saat *controller* di-dealokasi.
