...
Agar tidak membebani thread utama (UI thread) saat operasi masif terjadi (seperti saat sinkronisasi CloudKit menarik ratusan rekaman modifikasi sekaligus), VM ini menggunakan `Task` asinkron (Concurrency) yang dibatalkan (*cancelled*) jika *event* yang sama terjadi berurutan dalam periode singkat:
