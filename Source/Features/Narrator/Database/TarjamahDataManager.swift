...
class TarjamahGlobalManager {
    ...
    private func populateMenUFts(db: OpaquePointer?) {
        sqlite3_exec(db, "BEGIN", nil, nil, nil)
        var readStmt: OpaquePointer?
        sqlite3_prepare_v2(db, "SELECT uId, IsoName FROM men_u WHERE IsoName IS NOT NULL", -1, &readStmt, nil)
        var insertStmt: OpaquePointer?
        sqlite3_prepare_v2(db, "INSERT INTO fts_db.men_u_fts(rowid, IsoName_clean) VALUES (?, ?)", -1, &insertStmt, nil)
        let SQLITE_TRANSIENT = unsafeBitCast(OpaquePointer(bitPattern: -1), to: sqlite3_destructor_type.self)

        while sqlite3_step(readStmt) == SQLITE_ROW {
            let uid = sqlite3_column_int(readStmt, 0)
            let isoNameClean = extractCleanString(stmt: readStmt, column: 1)
    ...
    private func populateMenBFts(db: OpaquePointer?) {
        sqlite3_exec(db, "BEGIN", nil, nil, nil)
        var readStmt: OpaquePointer?
        sqlite3_prepare_v2(db, "SELECT Id, Name FROM men_b WHERE Name IS NOT NULL AND Name != ''", -1, &readStmt, nil)
        var insertStmt: OpaquePointer?
        sqlite3_prepare_v2(db, "INSERT INTO fts_db.men_b_fts(rowid, Name_clean) VALUES (?, ?)", -1, &insertStmt, nil)
        let SQLITE_TRANSIENT = unsafeBitCast(OpaquePointer(bitPattern: -1), to: sqlite3_destructor_type.self)

        while sqlite3_step(readStmt) == SQLITE_ROW {
            let uid = sqlite3_column_int(readStmt, 0)
            if let text = readColumnString(stmt: readStmt, column: 1) {
                let nameClean = text.stemArabicLight10()
    ...
    private func streamProcessedResults(
        ...
        for (index, mutT) in items.enumerated() {
            if index % 10 == 0 {
                if stopFlag() || Task.isCancelled { break }
                await pauseController?.waitIfPaused()
            }
