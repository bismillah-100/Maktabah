...
enum AnnotationJsonSerializer {
    ...
    static func decode(from data: Data) throws -> [Annotation] {
        ...
        let array = extractArray(from: jsonObject)

        var result: [Annotation] = []
        result.reserveCapacity(array.count)

        for obj in array {
            result.append(parseAnnotation(from: obj))
        }

        return result
    }

    private static func extractArray(from jsonObject: Any) -> [[String: Any]] {
    ...
    private static func parseAnnotation(from obj: [String: Any]) -> Annotation {
        ...
        let tags: [String] = if let tagsStr = obj["tags"] as? String {
            tagsStr.split(separator: ",").map { String($0).trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }
        } else if let tagsArr = obj["tags"] as? [String] {
            tagsArr.map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }.filter { !$0.isEmpty }
        } else {
            []
        }
