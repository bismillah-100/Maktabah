...
extension String {
    ...
    func findRangeInOriginal(selectedText: String, approximateRange: NSRange) -> NSRange {
        ...
        if bestRange.location == NSNotFound {
            ...
            bestRange = findClosestMatch(in: nsNoHarakat, target: cleanSelected, searchRange: searchRange, approximateLocation: approximateRange.location)
        }

        // 3. Fallback: search entire string for closest match
        if bestRange.location == NSNotFound {
            let fullRange = NSRange(location: 0, length: totalLen)
            bestRange = findClosestMatch(in: nsNoHarakat, target: cleanSelected, searchRange: fullRange, approximateLocation: approximateRange.location)
        }
    ...
    private func findClosestMatch(
        in text: NSString,
        target: String,
        searchRange: NSRange,
        approximateLocation: Int
    ) -> NSRange {
        ...
        while curRange.length > 0 {
            let match = text.range(of: target, options: .diacriticInsensitive, range: curRange)
            guard match.location != NSNotFound else { break }

            let dist = abs(match.location - approximateLocation)
            if dist < minDistance {
                minDistance = dist
                bestRange = match
                if dist == 0 { break }
