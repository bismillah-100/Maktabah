...
extension String {
    ...
    private static func appendCleanedCharacter(
        ...
        replRLen: Int,
        charLen: Int,
        finalString: inout String,
        coloredRanges: inout [NSRange],
        nextDelta: inout Int
    ) {
        if character == "{" {
            let symbolStart = finalString.utf16.count
            finalString += replL
            nextDelta += (replLLen - charLen)
            coloredRanges.append(NSRange(location: symbolStart, length: replLLen))
        } else if character == "}" {
            let symbolStart = finalString.utf16.count
            finalString += replR
            nextDelta += (replRLen - charLen)
            coloredRanges.append(NSRange(location: symbolStart, length: replRLen))
        } else {
            let symbolStart = finalString.utf16.count
    ...
    private func parseCleanedTextAndEvents() -> CleanedTextParseOutput {
        ...
        let replLLen = replL.utf16.count
        let replRLen = replR.utf16.count
        ...
        while index < endIndex {
            ...
            if removableCharacters.contains(character) {
                ...
                Self.appendCleanedCharacter(
                    ...
                    replRLen: replRLen,
                    charLen: charLen,
                    finalString: &state.finalString,
                    coloredRanges: &state.coloredRanges,
                    nextDelta: &nextDelta
                )
                index = self.index(after: index)
            }
